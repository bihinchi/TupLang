...
def create_scope(semdata, name):
    semdata.symtbl[name] = {}
    semdata.symtbl[name]["value"] = dict()  # identifier -> value
    semdata.symtbl[name]["no_value"] = set()  # node with .eval
  ...
  if before_func:
    ...
    if not err is None:
      if hasattr(node, "lineno"):
        ...
        err = "Line {}: {}".format(node.lineno, err)
      print(err)
      sys.exit()
